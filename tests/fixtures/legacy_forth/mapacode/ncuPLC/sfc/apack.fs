...
\ Frequency response
0.94e 0 gain-array!         \   <5HZ   表示<5HZ時,增益為0.96倍
0.79e 1 gain-array!         \   5HZ  
0.83e 2 gain-array!         \   10HZ
0.95e  3 gain-array!        \   15HZ
1.16e  4 gain-array!        \   20HZ
1.45e  5 gain-array!        \   25HZ
1.82e 6 gain-array!         \   30HZ
2.1e 7 gain-array!          \   35HZ
2.6e 8 gain-array!          \   40HZ
2.9e  9 gain-array!         \   45HZ
3.04e 10 gain-array!        \   50HZ
3.04e  11 gain-array!       \   >50HZ
...
\ modbus參數 想得到20v 則要輸入9.5v
: apk-vl! ( F: vl -- ) 
    fdup 0.475e f* target-v f!
    0 apks-vl fparam! ; 
: apk-vl@ ( F: -- vl ) 
    0 apks-vl fparam@ ;
...
\ modbus參數 想得到1000A 則要輸入9.5v

: apk-ip! ( ip -- ) 
    dup s>f 0.0095e f* target-i f!
    0 apks-ip param! ; 
: apk-ip@ ( -- ip ) 
    0 apks-ip param@ ; 
...
\ 振幅允差
fvariable amp-tolerance1 0.02e amp-tolerance1 f! \ 界線1 較窄的 
fvariable amp-tolerance2 0.5e amp-tolerance2 f!  \ 界線2 較寬的 
\ amp-modulation ： apk-va@ / real-trough
\ amp-modulation >2.5                                   表示目前外在環境有問題,不去隨意變更命令
\ 2.5>amp-modulation>1+amp-tolerance2                   表示實際位置較高, 去增加airpump去將振動軸實際位置往下推
\ 1.5>amp-modulation>1+amp-tolerance1                   表示實際位置偏高, 目前將命令增加1.03倍
\ 1+amp-tolerance1 >amp-modulation>1-amp-tolerance1     表示位置達到所需, 故不需要修改命令
\ 1-amp-tolerance1>amp-modulation>1-amp-tolerance2      表示實際位置偏低, 若有airpump則慢慢降低直到變成0,0以後將命令減少1.05倍
\ 1-amp-tolerance2>amp-modulation                       表示目前外在環境有問題,不去隨意變更命令
...
: modify-amp-updata-input ( -- )
    ...
    fdup 2.5e f>  if
        ...
        fdup 1e amp-tolerance2 f@ f+ f> if
            air-pump @ 32767 < if
                \ 一次約增加0.003bar
                10 ++air-pump
                amp-ready off
            ...
            fdup 1e amp-tolerance1 f@ f+ f> if
                2 group! 
...
: .modify-amp-count         ( -- )
    ...
    ." air_pump|" air-pump @ s>f 3276.7e f/ 4 3 f.r  cr
    ." amp_modulation|" amp-modulation f@ 4 2 f.r  cr
;
