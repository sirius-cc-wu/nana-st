...
: all-drive-ready?
  \ 檢查所有軸是否 drive on 成功
  drive-x drive-on?
  drive-y drive-on? and
  drive-z drive-on? and
  drive-v drive-on? and
  if
    TRUE
  else
    FALSE
...
: delay2on-x
  ['] reset-drive-delay elapsed 500 >  if
    ...
    drive-y drive-fault? not and
    drive-z drive-fault? not and
    drive-v drive-fault? not and
  else
    FALSE
  then
