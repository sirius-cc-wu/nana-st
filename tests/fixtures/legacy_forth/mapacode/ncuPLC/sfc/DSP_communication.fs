...
: DSP-check-header ( -- flag )
    ECM-DSP-ch-slv uart-rx-len@ DSP-header-size < invert
    if
        false DSP-header-size 0     ( false size 0 )
        begin                       ( flag size n )
            over over >
            if
                DSP-header over byte@ 1 ECM-DSP-ch-slv uart-data@ =
            else
                false
            then
        while                       ( flag size n )
...
: TDU>DSP ( -- )
    ...
    while                           ( size n )
        ...
        1 ECM-DSP-ch-slv uart-data! ( size n )
        1 +                         ( size n+1 )
    repeat                          ( size n )
    drop drop                       ( )
;
...
: DSP>RDU-buf
    ...
    begin                           ( size n )
        over over >                 ( size n flag )
    while                           ( size n )
        1 ECM-DSP-ch-slv uart-data@ ( size n val )
...
: DSP-S1
    ECM-DSP-ch-slv uart-ready?
    DSP-S1-done @ not and
    if
        6 ECM-DSP-ch-slv uart-baud!
        3 ECM-DSP-ch-slv uart-frame!
        DSP-S1-done on
    then
;
...
: DSP-S6
    ECM-DSP-ch-slv uart-rx-len@ DSP-RDU-size < invert
    if
        DSP>RDU-buf
        DSP-S6-done on
    then
