...
\ 2:表示standby
\ 3:表示放電中
0 constant munk-not-connect
1 constant munk-poweroff
2 constant munk-standBy
3 constant munk-cutting
variable munk-st munk-not-connect munk-st !
...
\ commands: 0=munk-none, 1=read, 2=write
...
: .system-status ( -- )
    ." system_ready|" munk-ready? 0 .r cr
;
...
: .munk-status ( -- )
    ...
    case
        ...
        2 of
            ." munk_act_ramp|"
            act-ramp-sec@ 0 .r ." ."
            act-ramp-start-perc@ 0 .r ." ."
            act-ramp-final-perc@ 0 .r cr
...
variable munk-wave-init-done
variable munk-wave-go
variable munk-wave-op
variable munk-wave-count
0 constant munk-pw-on
1 constant munk-pw-off
2 constant munk-clear-err

: +munk-pw-on
    munk-wave-go on
    munk-pw-on munk-wave-op !
;

: +munk-pw-off
    munk-wave-go on
    munk-pw-off munk-wave-op !
;

: +munk-clear-err
    munk-wave-go on
    munk-clear-err munk-wave-op !
;
...
: munk-wave
    op-mode-profinet? if
        munk-wave-count @ 0= if
            munk-wave-op @      
            case
                munk-pw-on of  +ready-to-op  endof
                munk-pw-off of  -switch-on  endof
                munk-clear-err of  +ack-faults  endof
                ." error|Munk command fail. ;A1540 " dup  0 .r cr
            endcase
        then
        munk-wave-count @ 5000 = if
            munk-wave-op @      
            case
                munk-pw-on of  -ready-to-op  endof
                munk-pw-off of  +switch-on  endof
        ...
        munk-wave-go off
        6000 munk-wave-count !
        ." error|Munk not in profinet. ;A1540 " cr
    then
;
step munk-wave
...
: munk-wave-init-done?
    munk-wave-init-done @
;
transition munk-wave-init-done?

: munk-wave-go?
    munk-wave-go @
;
transition munk-wave-go?

: munk-wave-end?
    munk-wave-count @ 5500 >=
;
transition munk-wave-end?

\ ==================== links ====================
' munk-wave-init            ' munk-wave-init-done?      -->
' munk-wave-init-done?      ' munk-wave-idle            -->
' munk-wave-idle            ' munk-wave-go?             -->
' munk-wave-go?             ' munk-wave                 -->
' munk-wave                 ' munk-wave-end?            -->
' munk-wave-end?            ' munk-wave-init            -->

\ --------------------munk-state steps --------------------
\ 初始化
variable munk-state-done
: munk-state ( -- )
    munk-configured @ not if
        munk-not-connect munk-st !
    else
        rectifier-on? if
            munk-cutting munk-st !
        else
            rectifier-ready-op? if
                munk-standBy munk-st !
            else
                munk-poweroff munk-st !
            then
        then
    then
    munk-state-done on
...
\ -------------------- transitions --------------------
\ 初始化完成？
: munk-state-done? ( -- flag )
    munk-state-done @
;
transition munk-state-done?

\ -------------------- links --------------------
' munk-state ' munk-state-done? -->
' munk-state-done? ' munk-state -->
...
: .munk-st
    ." munk| " munk-st @ . cr
    munk-configured @ if
        ." munk_v| " act-v1@ s>f 0.001e f* 4 1 f.r cr
        ." munk_i| " act-a1@ s>f 4 1 f.r cr 
    then
;
marker -work