...
type: "Vision"
title: "Vision: NanaST"
description: "NanaST gives BNC/control developers a Structured Text path targeting the Forth-2012 standard."
status: "approved"
tags: [vision, governance, nanast, forth-2012]
---
...
target feature specification must define for a Forth-2012-conforming execution
environment.

It owns defining its ST subset, source-aware diagnostics, the future
ST-to-Forth-2012 target contract, and compiler-correctness evidence. It does
not own a target Forth runtime implementation, BNC device integration, or BNC
control policy.
...
- **Forth-2012 is the target standard:** A target feature specification must
  define how a specified ST program translates to an artifact that uses the
  required Forth-2012 semantics and words. Keep the artifact and host contract
  explicit and testable. A target-specific extension requires an explicit,
  separately verified boundary; it must not become an implicit compiler
  dependency.
...
- **Real-time claims require target evidence:** Do not infer a timing, memory,
  safety, or motion claim merely because one BNC deployment uses a particular
  Forth implementation. A target feature specification and recorded evidence
  must establish any such claim for a generated control case.
...
- **BNC owns BNC integration:** Keep EtherCAT mapping, DTC persistence and
  ...
  and control safety policy in BNC. Keep NanaST's Forth-2012 target boundary
  narrow and device-neutral.
- **Scope grows from demonstrated need:** Add language features or
  Forth-2012 target capabilities only when a named BNC control case and
  verification evidence justify them.
...
- An implementation-specific Forth backend that does not preserve the
  Forth-2012 target contract.
...
- A change aligns when it preserves the semantics and diagnostics specified for
  that target feature and supports incremental coexistence with existing BNC
  Forth programs.
...
- A change should be resisted when it adds Wasm, Wasmtime, a direct native-code
  backend, broad IEC syntax, or an IDE without a later approved vision revision.

## Boundary Cases

- **A contribution proposes a Wasm or native-code backend:** Resist it.
  NanaST's approved target compatibility boundary is Forth-2012.
- **A contribution proposes a target for a specific Forth implementation:**
  Accept it only when the emitted artifact remains Forth-2012-compatible and
  any required extension is an explicit, verified host boundary. Resist an
  implementation-only compiler backend.
...
- Authority and status: Sirius Wu approved this revision on 2026-09-06. It
  selects Forth-2012 as NanaST's target compatibility boundary, superseding the
  rtForth-specific target selection at revision `466fb04`. It continues to
  supersede the Wasm compiler target established by the v0.1 baseline at
  `a63b5ca` and the Wasmtime runner direction approved at commit `1501430`.
...
- Evidence: Botnana Control revision `4ef5e97` uses `rtforth` in
  `motion/Cargo.toml` and documents a real-time Forth VM. This supports an
  rtForth deployment as one compatibility candidate; it does not make rtForth
  NanaST's exclusive target or establish a target contract or real-time claim
  for generated ST programs.
- Open questions: The emitted Forth-2012 artifact format, required standard
  words, optional target-extension boundary, BNC host contract, lifecycle, and
  target timing and memory acceptance evidence require a new approved feature
  specification.
